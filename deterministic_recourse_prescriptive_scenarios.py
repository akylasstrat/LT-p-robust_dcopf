...
scen_robust_opf_model = Scenario_Robust_OPF(num_uncertainties, num_scen, w_scenarios, grid, c_viol = c_viol)
optimizer = torch.optim.Adam(scen_robust_opf_model.parameters(), lr = 1e-2)
...
output = pd.DataFrame(data = [], columns = ['DA_cost', 'RT_cost'], index = ['DD_box', 'CD_poly'])

output.loc['DD_box']['DA_cost'] = box_scen_solution['da_cost']
output.loc['DD_box']['RT_cost'] = RT_Clearing(grid, 2, c_viol).forward(box_scen_solution, torch.FloatTensor(test_errors)).mean()

output.loc['CD_poly']['DA_cost'] = cost_driven_solution['da_cost']
output.loc['CD_poly']['RT_cost'] = RT_Clearing(grid, 2, c_viol).forward(cost_driven_solution, torch.FloatTensor(test_errors)).mean()
